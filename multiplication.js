const createTable = () => {
  ...
  table.innerHTML = "";
  const number = document.getElementById("txtInput");
  const numberValue = number.value;

  const inputFactor = document.getElementById("inputFactor");
  inputFactor.innerHTML = numberValue
  
  const factor = document.getElementById("factor");
  factor.innerHTML = "x" + numberValue

  const maxValue = document.getElementById("maxValue");
  maxValue.innerHTML = numberValue * 10
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    spanFactor1_i.textContent = numberValue;
    ...
    spanResult_i.textContent = numberValue * i;
